...
(function() {
  ...
  function activate(btn, animate) {
    ...
    if (!animate) requestAnimationFrame(function() { slider.style.transition = ''; });
    ...
    history.pushState(null, '', '?view=' + btn.dataset.view);
  }

  // Set initial view from URL param
  var view = new URLSearchParams(window.location.search).get('view');
  var initialBtn = toggle.querySelector(view ? '[data-view="' + view + '"]' : '.toggle-btn.active');
  requestAnimationFrame(function() { requestAnimationFrame(function() { activate(initialBtn, false); }); });
